...
def test_raster_datum_sep():
    ...
    vr.set_input_datum('nad83')
    vr.set_output_datum('mllw')
    vr.get_datum_sep()

    assert vr.raster_vdatum_sep[0][0] == approx(29.162, abs=0.001)
...
def test_raster_forced_input_vertical_datum():
    ...
    layers, layernames, layernodata = vr.transform_raster('mllw', 'geoid12b', allow_points_outside_coverage=True)
...
def test_raster_write_to_geotiff():
    ...
    layers, layernames, layernodata = vr.transform_raster('navd88', 'mllw', allow_points_outside_coverage=True, output_filename=output_filename)
