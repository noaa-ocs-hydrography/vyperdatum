...
def test_vertical_pipeline_crs():
    cs = VerticalPipelineCRS(vdatum_version_string='vdatum_4.1.2_20201203', vert_datum_name="NOAA Chart Datum")
    cs.add_pipeline(
...
def test_transformation_inv_nad83():
    ...
    cstwo = VerticalPipelineCRS(vdatum_version_string='vdatum_4.1.2_20201203', vert_datum_name="NAD83(2011)_ellipse")
    pipe = get_transformation_pipeline(cs, cstwo, "TXlaggal01_8301", 'vdatum_4.1.2_20201203')
...
def test_transformation_tss():
    cs = VerticalPipelineCRS(vdatum_version_string='vdatum_4.1.2_20201203', vert_datum_name="tss")
    cs.add_pipeline(
        "proj=pipeline step proj=vgridshift grids=core\\geoid12b\\g2012bu0.gtx step proj=vgridshift grids=REGION\\tss.gtx",
        "TXlagmat01_8301")

    cstwo = VerticalPipelineCRS(vdatum_version_string='vdatum_4.1.2_20201203', vert_datum_name="noaa chart datum")
    cstwo.add_pipeline(
        "proj=pipeline step proj=vgridshift grids=core\\geoid12b\\g2012bu0.gtx step proj=vgridshift grids=REGION\\tss.gtx step proj=vgridshift grids=REGION\\mllw.gtx",
        "TXlagmat01_8301")

    pipe = get_transformation_pipeline(cs, cstwo, "TXlagmat01_8301", 'vdatum_4.1.2_20201203')
...
def test_transformation_unsupported_name():
    ...
    cstwo = VerticalPipelineCRS(vdatum_version_string='vdatum_4.1.2_20201203', vert_datum_name="some_bs")
    cstwo.add_pipeline(
        "proj=pipeline step proj=vgridshift grids=core\\geoid12b\\g2012bu0.gtx step proj=vgridshift grids=REGION\\tss.gtx step proj=vgridshift grids=REGION\\mllw.gtx",
        "TXlaggal01_8301")
...
def test_vyperpipeline_add_vertcrs_no_pipeline():
    ...
    cs = VyperPipelineCRS(vdatum_version_string='vdatum_4.1.2_20201203')
...
def test_vyperpipeline_add_vertcrs_then_pipeline_then_pipeline():
    ...
    cs = VyperPipelineCRS('vdatum_4.1.2_20201203')
    cs.set_crs("NOAA Chart Datum")
...
def test_vyperpipeline_set_compound_wkt_on_instantiation():
    ...
    cs = VyperPipelineCRS('vdatum_4.1.2_20201203', new_crs=compound_wkt)
    assert cs.horizontal.to_epsg() == 26918
    assert cs.is_valid
...
def test_3d_to_compound():
    cs = VyperPipelineCRS('vdatum_4.1.2_20201203', new_crs=6319, regions=['MENHMAgome23_8301'])
    assert cs.is_valid
