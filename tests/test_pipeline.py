...
def test_get_regional_pipeline_upperlower():
    pipe = get_regional_pipeline('Ellipse', 'TSS', 'CAORblan01_8301', 'vdatum_4.1.2')
    assert pipe == get_regional_pipeline('ellipse', 'tss', 'CAORblan01_8301', 'vdatum_4.1.2')
...
def test_get_regional_pipeline_tss_nad83():
    pipe = get_regional_pipeline('tss', 'ellipse', 'CAORblan01_8301', 'vdatum_4.1.2')
    assert pipe.count('+step +inv +proj') == 1
    assert pipe.count('+step +proj') == 1
    assert pipe.count('gtx') == 2
    transformer = Transformer.from_pipeline(pipe)
...
def test_get_regional_pipeline_mllw():
    pipe = get_regional_pipeline('ellipse', 'mllw', 'CAORblan01_8301', 'vdatum_4.1.2')
...
def test_get_regional_pipeline_mhw():
    pipe = get_regional_pipeline('ellipse', 'mhw', 'CAORblan01_8301', 'vdatum_4.1.2')
    assert pipe.count('+step +proj') == 2
    assert pipe.count('+step +inv +proj') == 1
    assert pipe.count('gtx') == 3
...
def test_get_regional_pipeline_null():
    pipe = get_regional_pipeline('mllw', 'mllw', 'CAORblan01_8301', 'vdatum_4.1.2')
    assert pipe is None
