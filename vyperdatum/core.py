...
def get_uncertainty(in_uncertainty, in_crs, out_crs):
    """"
    Parse the reported operation uncertainty or get the spatially varying uncertainty and report the change from the input to the
    output with the same dimensions as the provided input uncertainty.
...
def get_intersecting_regions(input_shape):
    """
    Provided a geometry, find the available model regions and provide back the names as a list.

    Maybe this should also provide back if the model region contains a dataset or if it just intersects?
